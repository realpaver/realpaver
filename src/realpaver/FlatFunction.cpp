...
bool FlatFunction::rDiff()
{
   ...
   for (int i = (int)nb_ - 1; i >= 0; --i)
   {
      ...
      case FlatSymbol::Sqr:
         // d(u^2)/du = 2u
         rdv_[arg_[i][1]] = Double::mul(2.0, Double::mul(rval_[arg_[i][1]], rdv_[i]));
         break;
      ...
      case FlatSymbol::Pow: {
         ...
         int e = (int)arg_[i][2];
         rdv_[arg_[i][1]] =
             Double::mul(Double::mul(e, rdv_[i]), Double::pow(rval_[arg_[i][1]], e - 1));
      }
      break;
      ...
      case FlatSymbol::Cos:
         // d(cos(u))/du = -sin(u)
         rdv_[arg_[i][1]] =
             Double::usb(Double::mul(rdv_[i], Double::sin(rval_[arg_[i][1]])));
         break;

      case FlatSymbol::Sin:
         // d(sin(u))/du = cos(u)
         rdv_[arg_[i][1]] = Double::mul(rdv_[i], Double::cos(rval_[arg_[i][1]]));
         break;

      case FlatSymbol::Tan:
         // d(tan(u))/du = 1+tan^2(u)
         rdv_[arg_[i][1]] = Double::mul(rdv_[i], Double::add(1.0, Double::sqr(rval_[i])));
         break;

      case FlatSymbol::Cosh:
         // d(cosh(u))/du = sinh(u)
         rdv_[arg_[i][1]] = Double::mul(rdv_[i], Double::sinh(rval_[arg_[i][1]]));
         break;

      case FlatSymbol::Sinh:
         // d(sinh(u))/du = cosh(u)
         rdv_[arg_[i][1]] = Double::mul(rdv_[i], Double::cosh(rval_[arg_[i][1]]));
         break;

      case FlatSymbol::Tanh:
         // d(tanh(u))/du = 1-tanh^2(u)
         rdv_[arg_[i][1]] = Double::mul(rdv_[i], Double::sub(1.0, Double::sqr(rval_[i])));
         break;
      }
   }
...
FlatFunUniCreator::FlatFunUniCreator(FlatFunction *f, const IntervalBox &B, Variable v)
    : f_(f)
    , B_(B)
    , v_(v)
{
