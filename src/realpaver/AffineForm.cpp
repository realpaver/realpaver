...
AffineForm::AffineForm() : c_(0.0), l_(), e_(0.0) {}

AffineForm::AffineForm(const Interval &x) : c_(x), l_(), e_(0.0) {
  ASSERT(!x.isEmpty(), "Affine form invalid (empty)");
  ASSERT(!x.isInf(), "Affine form invalid (inf)");
}

AffineForm::AffineForm(const double &a) : c_(a), l_(), e_(0.0) {
...
AffineForm::AffineForm(const Interval &a0,
                       const std::initializer_list<Interval> &A,
                       const std::initializer_list<int> &V, const Interval &e)
    : c_(a0), l_(), e_(e) {
...
void AffineForm::setEmpty() { c_ = Interval::emptyset(); }

bool AffineForm::isEmpty() const { return c_.isEmpty(); }

size_t AffineForm::nbLinearTerms() const { return l_.size(); }
...
bool AffineForm::isInf() const { return e_.isInf(); }

bool AffineForm::usesMinrange() { return MINRANGE; }

void AffineForm::useMinrange(bool b) { MINRANGE = b; }

void AffineForm::useChebyshev(bool b) { MINRANGE = b; }

const Interval &AffineForm::constantTerm() const { return c_; }

const Interval &AffineForm::errorTerm() const { return e_; }
...
void AffineForm::minrangeSqrt(const Interval &x, Interval &alpha,
                              Interval &dzeta, Interval &delta) {
...
void AffineForm::chebyshevSqrt(const Interval &x, Interval &alpha,
                               Interval &dzeta, Interval &delta) {
  Interval a(x.left()), b(x.right()), fa = sqrt(a), fb = sqrt(b);
...
void AffineForm::minrangeUdiv(const Interval &x, Interval &alpha,
                              Interval &dzeta, Interval &delta) {
...
void AffineForm::chebyshevUdiv(const Interval &x, Interval &alpha,
                               Interval &dzeta, Interval &delta) {
...
void AffineForm::minrangeExp(const Interval &x, Interval &alpha,
                             Interval &dzeta, Interval &delta) {
...
void AffineForm::chebyshevExp(const Interval &x, Interval &alpha,
                              Interval &dzeta, Interval &delta) {
  Interval a(x.left()), b(x.right()), fa = exp(a), fb = exp(b);
...
void AffineForm::minrangeLog(const Interval &x, Interval &alpha,
                             Interval &dzeta, Interval &delta) {
...
void AffineForm::chebyshevLog(const Interval &x, Interval &alpha,
                              Interval &dzeta, Interval &delta) {
  Interval a(x.left()), b(x.right()), fa = log(a), fb = log(b);
...
void AffineForm::minrangeCos(const Interval &x, Interval &alpha,
                             Interval &dzeta, Interval &delta) {
...
void AffineForm::chebyshevCos(const Interval &x, Interval &alpha,
                              Interval &dzeta, Interval &delta) {
  Interval cx = cos(x);
...
void AffineForm::minrangeSin(const Interval &x, Interval &alpha,
                             Interval &dzeta, Interval &delta) {
...
void AffineForm::chebyshevSin(const Interval &x, Interval &alpha,
                              Interval &dzeta, Interval &delta) {
...
void AffineForm::minrangeTan(const Interval &x, Interval &alpha,
                             Interval &dzeta, Interval &delta) {
...
void AffineForm::chebyshevTan(const Interval &x, Interval &alpha,
                              Interval &dzeta, Interval &delta) {
  Interval tx = tan(x);
  ...
  Interval c =
      (tx.isPositive() ? acos(1.0 / sqrt(alpha)) : -acos(1.0 / sqrt(alpha)));
  c -= k * Interval::pi();
  Interval fc = tan(c);
...
void AffineForm::minrangeCosh(const Interval &x, Interval &alpha,
                              Interval &dzeta, Interval &delta) {
...
void AffineForm::chebyshevCosh(const Interval &x, Interval &alpha,
                               Interval &dzeta, Interval &delta) {
  Interval a(x.left()), b(x.right()), fa = cosh(a), fb = cosh(b);
...
void AffineForm::minrangeSinh(const Interval &x, Interval &alpha,
                              Interval &dzeta, Interval &delta) {
...
void AffineForm::chebyshevSinh(const Interval &x, Interval &alpha,
                               Interval &dzeta, Interval &delta) {
  if (x.strictlyContainsZero()) {
...
void AffineForm::minrangeTanh(const Interval &x, Interval &alpha,
                              Interval &dzeta, Interval &delta) {
...
void AffineForm::chebyshevTanh(const Interval &x, Interval &alpha,
                               Interval &dzeta, Interval &delta) {
  if (x.strictlyContainsZero()) {
  ...
  Interval c = (x.isPositive() ? acosh(sqrt(1.0 / alpha))
                               : -acosh(sqrt(1.0 / alpha))),
           fc = tanh(c);
...
AffineForm::iterator AffineForm::begin() { return l_.begin(); }

AffineForm::iterator AffineForm::end() { return l_.end(); }

int AffineForm::var(const iterator &it) { return it->var; }

const Interval &AffineForm::itv(const iterator &it) { return it->itv; }

AffineForm::const_iterator AffineForm::cbegin() const { return l_.cbegin(); }

AffineForm::const_iterator AffineForm::cend() const { return l_.cend(); }

int AffineForm::var(const const_iterator &it) { return it->var; }

const Interval &AffineForm::itv(const const_iterator &it) { return it->itv; }
