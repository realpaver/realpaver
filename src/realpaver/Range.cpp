...
Range Range::roundOutward(const Interval &x)
{
   ...
   Integer lo = Integer::floor(x.left()), up = Integer::ceil(x.right());
...
Range Range::roundInward(const Interval &x)
{
   ...
   Integer lo = Integer::ceil(x.left()), up = Integer::floor(x.right());
...
Range &Range::operator*=(const Range &other)
{
   if (!isEmpty())
   {
      ...
      {
         Integer a1 = l_ * other.l_, a2 = l_ * other.r_, a3 = r_ * other.l_,
                 a4 = r_ * other.r_;
...
Range sqrPX(const Range &x, const Range &y)
{
   ...
   {
      Integer a = sqrtCeil(y.l_), b = sqrtFloor(y.r_);
...
Range modPX(const Range &x, int n, const Range &y)
{
   ...
   Range xx = x & Range::positive(), yy = y & Range(0, n - 1);
   ...
   {
      Integer a = xx.l_ % n, b = xx.r_ % n, lo, up;
